...
class Modification extends \Opencart\System\Engine\Controller {
	...
	public function view(string &$route, array &$args): void {
		...
		if (substr($route, 0, 10) !== 'extension/') {
			$file = DIR_EXTENSION . 'ocmod/view/template/' . $route . '.twig';
		} else {
			$file = DIR_EXTENSION . 'ocmod/extension/' . substr($route, 10, strpos($route, '/', 10) - 10) . '/view/template/' . substr($route, strpos($route, '/', 10) + 1) . '.twig';
		}
