<?php

// Heading
$_['heading_title']        = 'Zamanlanmış Görevler';

// Text
$_['text_success']         = 'Başarılı: Zamanlanmış Görevler değiştirildi!';
$_['text_instruction']     = 'Görev Talimatları';
$_['text_list']            = 'Görev Listesi';
$_['text_cron_1']          = 'Cron İşleri, periyodik olarak çalıştırılan zamanlanmış görevlerdir. Sunucularınızı Zamanlanmış Görevleri kullanacak şekilde ayarlamak için <a href="https://docs.nettemarket.com/extension/cron/" target="_blank" class="alert-link">nettemarket dokümantasyon</a> sayfasını okuyabilirsiniz.';
$_['text_cron_2']          = 'Zamanlanmış görevinizi her saatte çalışacak şekilde ayarlamanız gerekir.';
$_['text_info']            = 'Görev Bilgileri';
$_['text_hour']            = 'Saat';
$_['text_day']             = 'Gün';
$_['text_month']           = 'Ay';

// Column
$_['column_code']          = 'Görev Kodu';
$_['column_cycle']         = 'Döngü';
$_['column_date_added']    = 'Ekleme Tarihi';
$_['column_date_modified'] = 'Düzenleme Tarihi';
$_['column_action']        = 'Eylem';

// entry
$_['entry_cron']           = 'Görev Adresi';
$_['entry_description']    = 'Açıklama';

// Error
$_['error_permission']     = 'Uyarı: Zamanlanmış Görevleri düzenleme iznine sahip değilsiniz!';